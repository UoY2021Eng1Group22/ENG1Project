...
public class ComputerBoat extends Boat {
  ...
  public void update(float deltaTime) {
    if (!recentCollision) {
      ...
      if (closest != null) {
        ...
        this.stamina = (this.stamina < this.maxStamina) ? this.regenerateStamina() + this.stamina :
            this.maxStamina;
      } else {
        ...
        if (!this.waiting) {
          ...
          if (this.stamina - diff > 0) {
            this.stamina -= diff;
            this.velocity
                .set(this.velocity.x, (this.speed + this.speed * this.velocityPercentage()));
          } else {
          ...
          this.stamina =
              (this.stamina < this.maxStamina) ? this.regenerateStamina() + this.stamina :
                  this.maxStamina;
        }
      }
  ...
  private float waitForRandomStamina() {
    return (float) ThreadLocalRandom.current()
        .nextDouble((double) this.maxStamina / 2, this.maxStamina);
  }

...
class FinalScreen implements Screen { // P2 - No need to be public
  ...
  public void render(float delta) {
    ...
    font.draw(this.game.getBatch(), "You have made it into the final!",
        (Gdx.graphics.getWidth() - layout.width) / 2, Gdx.graphics.getHeight() / 2);

    layout.setText(font, "Press Space to continue to the final!");
    font.draw(this.game.getBatch(), "Press Space to continue to the final!",
        (Gdx.graphics.getWidth() - layout.width) / 2, 100 / Settings.SCALAR + layout.height);

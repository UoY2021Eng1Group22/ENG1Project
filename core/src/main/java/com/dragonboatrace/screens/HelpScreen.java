...
public class HelpScreen implements Screen {
  ...
  public HelpScreen(MainMenuScreen callBack) {
    ...
    this.backButton =
        new Button(new Vector2(0, 0), "back_button_active.png", "back_button_inactive.png");
  ...
  public void render(float delta) {
    ...
    batch.begin();
    batch.draw(this.helpInfo, padding, currPos, Gdx.graphics.getWidth() - 2 * padding,
        (float) this.helpInfo.getHeight() / Settings.SCALAR);
    if (currPos == this.maxHeight) {
      font.draw(batch, "Scroll Down", (Gdx.graphics.getWidth() - layout.width) / 2,
          layout.height + 120f / Settings.SCALAR);
      batch.draw(this.downArrow, Gdx.graphics.getWidth() / 2f - 50f / Settings.SCALAR, 0,
          100f / Settings.SCALAR, 100f / Settings.SCALAR);
    }

...
public class BoatSelectScreen implements Screen {
  ...
  public BoatSelectScreen(DragonBoatRace game) {
    ...
    this.fastButton =
        new Button(new Vector2(spacing, 100), "fast_button_active.png", "fast_button_inactive.png");
    this.agileButton =
        new Button(new Vector2(spacing + (buttonWidth + spacing), 100), "agile_button_active.png",
            "agile_button_inactive.png");
    this.strongButton = new Button(new Vector2(spacing + (buttonWidth + spacing) * 2, 100),
        "strong_button_active.png", "strong_button_inactive.png");
    this.enduranceButton = new Button(new Vector2(spacing + (buttonWidth + spacing) * 3, 100),
  ...
  public void render(float delta) {
    ...
    font.draw(this.game.getBatch(), "Choose your Boat:",
        (Gdx.graphics.getWidth() - layout.width) / 2, Gdx.graphics.getHeight() - 100);
    ...
    if (this.fastButton.isHovering() && Gdx.input.isTouched()) {
      this.game.setScreen(new DifficultySelectScreen(this.game, "fast"));
    } else if (this.agileButton.isHovering() && Gdx.input.isTouched()) {
      this.game.setScreen(new DifficultySelectScreen(this.game, "agile"));
    } else if (this.strongButton.isHovering() && Gdx.input.isTouched()) {
      this.game.setScreen(new DifficultySelectScreen(this.game, "strong"));
    } else if (this.enduranceButton.isHovering() && Gdx.input.isTouched()) {
      this.game.setScreen(new DifficultySelectScreen(this.game, "endurance"));
    }
